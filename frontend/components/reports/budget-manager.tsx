...
export function BudgetForm({ onSave, onCancel, currentBudget }: BudgetFormProps) {
...
export function BudgetProgress({
    ...
    const getStatusBadge = () => {
        if (percentage >= 100) return { label: "Aşıldı", variant: "destructive" as const };
        if (percentage >= 80) return { label: "Dikkat", variant: "warning" as const };
        if (percentage >= 50) return { label: "Normal", variant: "secondary" as const };
        ...
        <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-2xl font-bold">{formatCurrency(spent, currency)}</p>
                </div>
            ...
            <div className="pt-2 border-t border-border">
                {isOverBudget ? (
                    ...
                    <p className="text-sm text-muted-foreground">
                        Kalan: <span className="font-medium text-foreground">{formatCurrency(remaining, currency)}</span>
                    </p>
                )}
            </div>
        </div>
...
export function useBudget() {
    ...
    return { budget, saveBudget, clearBudget, isLoading, monthLabel: formatMonthLabel(monthKey) };
}
